...
// src/services/aiIntegration/test/AIIntegrationService.spec.ts
...
describe("AIIntegrationService", () => {
  ...
  beforeEach(() => {
    ...
    import_openai2.default.mockImplementation(() => mockOpenAIInstance);
  ...
  it("should successfully process AI request and return parsed JSON", async () => {
    ...
    globalExpect(import_openai2.default).toHaveBeenCalledWith({
      apiKey: "test-api-key"
    });
    globalExpect(mockCreate).toHaveBeenCalledWith({
      model: "gpt-4o-mini",
      messages: [
        {
          role: "system",
  ...
  it("should handle JSON parsing errors and return error object", async () => {
    ...
    globalExpect(result).toEqual({
      error: true,
      message: globalExpect.stringContaining("Unexpected token"),
      code: 500 /* SYSTEM_ERROR */
    });
  });
  ...
  it("should handle complex JSON responses correctly", async () => {
    ...
    mockCreate.mockResolvedValue(mockResponse);
    const result = await service.execute(mockPrompt, mockTextContent);
    globalExpect(result).toEqual(complexJsonResponse);
  });
