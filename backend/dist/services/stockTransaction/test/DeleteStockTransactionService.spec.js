...
function internalSpyOn(obj, methodName, mock2) {
  assert(
  ...
  let ssr = false;
  accessType === "value" && originalDescriptor && !originalDescriptor.value && originalDescriptor.get && (accessType = "get", ssr = true, mock2 = originalDescriptor.get());
  let original;
  ...
  mock2 || (mock2 = original);
  let spy2 = wrap(createInternalSpy(mock2), mock2);
  ...
  return defineValue(state, "restore", restore), defineValue(state, "getOriginal", () => ssr ? original() : original), defineValue(state, "willCall", (newCb) => (state.impl = newCb, spy2)), reassign(
    ssr ? () => (setPototype(spy2, mock2), spy2) : spy2
  ), spies.add(spy2), spy2;
}
...
function wrap(mock2, original) {
  ...
  for (let key of properties) {
    ...
    getDescriptor(mock2, key) || define(mock2, key, descriptor);
  }
  return mock2;
}
...
function createSuiteCollector(name, factory = () => {
  ...
  const task = function(name2 = "", options = {}) {
    ...
    Error.stackTraceLimit = limit;
    if (handler2) {
      setFn(task2, withTimeout(withAwaitAsyncAssertions(withFixtures(runner, handler2, context), task2), timeout, false, stackTraceError, (_, error) => abortIfTimeout([context], error)));
    }
  ...
  const test5 = createTest(function(name2, optionsOrFn, optionsOrTest) {
    let { options, handler: handler2 } = parseArguments(optionsOrFn, optionsOrTest);
  ...
  suiteFn.each = function(cases, ...args) {
    ...
    return (name, optionsOrFn, fnOrOptions) => {
      ...
      cases.forEach((i, idx) => {
        ...
        if (fnFirst) {
          if (arrayOnlyCases) {
            suite2(formatTitle(_name, items, idx), () => handler2(...items), options);
          } else {
            suite2(formatTitle(_name, items, idx), () => handler2(i), options);
          }
        } else {
          if (arrayOnlyCases) {
            suite2(formatTitle(_name, items, idx), options, () => handler2(...items));
          } else {
            suite2(formatTitle(_name, items, idx), options, () => handler2(i));
          }
        }
      });
  ...
  suiteFn.for = function(cases, ...args) {
    ...
    return (name, optionsOrFn, fnOrOptions) => {
      ...
      cases.forEach((item, idx) => {
        suite(formatTitle(name_, toArray(item), idx), options, () => handler2(item));
      });
    };
  };
...
function createTaskCollector(fn2, context) {
  ...
  taskFn.each = function(cases, ...args) {
    ...
    return (name, optionsOrFn, fnOrOptions) => {
      ...
      cases.forEach((i, idx) => {
        ...
        if (fnFirst) {
          if (arrayOnlyCases) {
            test5(formatTitle(_name, items, idx), () => handler2(...items), options);
          } else {
            test5(formatTitle(_name, items, idx), () => handler2(i), options);
          }
        } else {
          if (arrayOnlyCases) {
            test5(formatTitle(_name, items, idx), options, () => handler2(...items));
          } else {
            test5(formatTitle(_name, items, idx), options, () => handler2(i));
          }
        }
      });
      this.setContext("each", void 0);
  ...
  taskFn.for = function(cases, ...args) {
    ...
    return (name, optionsOrFn, fnOrOptions) => {
      ...
      const { options, handler: handler2 } = parseArguments(optionsOrFn, fnOrOptions);
      cases.forEach((item, idx) => {
        const handlerWrapper = (ctx) => handler2(item, ctx);
        handlerWrapper.__VITEST_FIXTURE_INDEX__ = 1;
        handlerWrapper.toString = () => handler2.toString();
  ...
  taskFn.extend = function(fixtures) {
    ...
    return createTest(function(name, optionsOrFn, optionsOrTest) {
      ...
      const { handler: handler2, options } = parseArguments(optionsOrFn, optionsOrTest);
      const timeout = options.timeout ?? (runner === null || runner === void 0 ? void 0 : runner.config.testTimeout);
      originalWrapper.call(context2, formatName(name), handler2, timeout);
    }, _context);
  };
...
// node_modules/vitest-mock-extended/lib/esm/index.js
var __defProp3 = Object.defineProperty;
var __defNormalProp2 = (obj, key, value) => key in obj ? __defProp3(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
var __publicField2 = (obj, key, value) => __defNormalProp2(obj, typeof key !== "symbol" ? key + "" : key, value);
var Matcher = class {
  constructor(asymmetricMatch2, description) {
    this.asymmetricMatch = asymmetricMatch2;
    this.description = description;
    __publicField2(this, "$$typeof");
    __publicField2(this, "inverse");
...
var calledWithFn = ({ fallbackMockImplementation } = {}) => {
  const fn2 = fallbackMockImplementation ? vi.fn(fallbackMockImplementation) : vi.fn();
  let calledWithStack = [];
  fn2.calledWith = (...args) => {
    const calledWithFn2 = fallbackMockImplementation ? vi.fn(fallbackMockImplementation) : vi.fn();
    const mockImplementation = fn2.getMockImplementation();
    if (!mockImplementation || fn2.getMockImplementation()?.name === "implementation" || mockImplementation === fallbackMockImplementation) {
      fn2.mockImplementation((...args2) => checkCalledWith(calledWithStack, args2, fallbackMockImplementation));
      calledWithStack = [];
    }
...
var handler = (opts) => ({
  ...
  get: (obj, property) => {
    if (!(property in obj)) {
      ...
      const fn2 = calledWithFn({ fallbackMockImplementation: opts?.fallbackMockImplementation });
      if (opts?.deep && property !== "calls") {
        obj[property] = new Proxy(fn2, handler(opts));
...
// src/services/stockTransaction/test/DeleteStockTransactionService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
describe("DeleteStockTransactionService", () => {
  ...
  it("should delete a stock transaction successfully", async () => {
    ...
    const mockExistingTransaction = {
      ...
      unity_price: 10.5,
      total_price: 1050,
      purchased_date: /* @__PURE__ */ new Date("2023-01-01"),
    ...
    prisma_default.stockTransaction.findUnique.mockResolvedValue(mockExistingTransaction);
    prisma_default.stockTransaction.delete.mockResolvedValue(mockDeletedTransaction);
    const result = await service.execute(transactionId);
    globalExpect(prisma_default.stockTransaction.findUnique).toHaveBeenCalledWith({
      where: { id: transactionId }
    });
    globalExpect(prisma_default.stockTransaction.delete).toHaveBeenCalledWith({
      where: { id: transactionId }
    });
    globalExpect(result).toEqual(mockDeletedTransaction);
  });
  it("should return error when transaction not found", async () => {
    const transactionId = "nonexistent";
    const errorMessage = "Transaction not found";
    prisma_default.stockTransaction.findUnique.mockResolvedValue(null);
    try {
      await service.execute(transactionId);
    } catch (error) {
      globalExpect(error).toBeInstanceOf(BadRequestException);
      globalExpect(error.message).toBe(errorMessage);
      globalExpect(error.errorCode).toBe(500 /* SYSTEM_ERROR */);
  ...
  it("should delete transaction with different data types", async () => {
    ...
    const mockExistingTransaction = {
      ...
      unity_price: 8.75,
      total_price: 223.125,
      purchased_date: /* @__PURE__ */ new Date("2023-02-15"),
      created_at: /* @__PURE__ */ new Date(),
      updated_at: /* @__PURE__ */ new Date()
    };
    prisma_default.stockTransaction.findUnique.mockResolvedValue(mockExistingTransaction);
    prisma_default.stockTransaction.delete.mockResolvedValue(mockExistingTransaction);
    const result = await service.execute(transactionId);
    globalExpect(result).toEqual(mockExistingTransaction);
  });
});
