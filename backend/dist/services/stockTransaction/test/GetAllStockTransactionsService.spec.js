...
// src/services/stockTransaction/test/GetAllStockTransactionsService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
describe("GetAllStockTransactionsService", () => {
  ...
  it("should get all stock transactions with default pagination", async () => {
    const mockTransactions = [
      ...
      },
      {
        ...
        unity_price: 5.25,
        total_price: 262.5,
        purchased_date: /* @__PURE__ */ new Date("2023-01-02"),
        created_at: /* @__PURE__ */ new Date(),
        updated_at: /* @__PURE__ */ new Date(),
        product: {
          id: "product456",
    ...
    prisma_default.stockTransaction.findMany.mockResolvedValue(mockTransactions);
    prisma_default.stockTransaction.count.mockResolvedValue(2);
    const result = await service.execute();
    globalExpect(prisma_default.stockTransaction.findMany).toHaveBeenCalledWith({
      where: {},
      include: {
        product: true
      },
    ...
    globalExpect(prisma_default.stockTransaction.count).toHaveBeenCalledWith({
      where: {}
    });
  ...
  it("should get stock transactions with custom pagination", async () => {
    ...
    prisma_default.stockTransaction.findMany.mockResolvedValue(mockTransactions);
    prisma_default.stockTransaction.count.mockResolvedValue(25);
  ...
  it("should filter stock transactions by product name", async () => {
    ...
    prisma_default.stockTransaction.findMany.mockResolvedValue(mockTransactions);
    prisma_default.stockTransaction.count.mockResolvedValue(1);
  ...
  it("should handle empty results", async () => {
    prisma_default.stockTransaction.findMany.mockResolvedValue([]);
    prisma_default.stockTransaction.count.mockResolvedValue(0);
    const result = await service.execute();
    globalExpect(result).toEqual({
      stockTransactions: [],
      total: 0,
  ...
  it("should order transactions by purchased_date desc", async () => {
    ...
    globalExpect(result.stockTransactions).toEqual(mockTransactions);
  });
  it("should include product information in results", async () => {
    ...
    globalExpect(result.stockTransactions[0].product).toBeDefined();
    globalExpect(result.stockTransactions[0].product.name).toBe("Test Product");
  });
});
