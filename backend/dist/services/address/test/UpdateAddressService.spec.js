...
// src/services/address/test/UpdateAddressService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
describe("UpdateAddressService", () => {
  ...
  it("should update an address", async () => {
    const mockedAddress = {
      ...
      reference_point: "Ponto de Refer\xEAncia 1",
    ...
    prisma_default.address.update.mockResolvedValue(updatedAddress);
    const result = await updateAddressService.execute(mockedAddress);
    globalExpect(prisma_default.address.update).toHaveBeenCalledWith({
      where: {
        id: mockedAddress.id
      },
      data: mockedAddress
    });
    globalExpect(result).toEqual({ status: "Address successfully updated", address: updatedAddress });
  });
});
