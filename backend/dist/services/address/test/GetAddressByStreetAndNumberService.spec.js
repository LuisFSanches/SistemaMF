...
// src/services/address/test/GetAddressByStreetAndNumberService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
describe("GetAddressByStreetAndNumber", () => {
  let getAddressByStreetAndNumber;
  let prisma;
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("should get an address by street and street number", async () => {
    ...
    const mockAddressData = {
      ...
      street_number: "100",
      complement: "Apto 202",
      reference_point: "Pr\xF3ximo \xE0 padaria",
      neighborhood: "Centro",
      city: "S\xE3o Paulo",
      state: "SP",
    ...
    prisma_default.address.findFirst.mockResolvedValue(mockAddressData);
    const result = await getAddressByStreetAndNumber.execute(client_id, street, streetNumber);
    globalExpect(prisma.address.findFirst).toHaveBeenCalledWith({
      where: {
        street,
  ...
  it("should return null when address is not found", async () => {
    ...
    prisma_default.address.findFirst.mockResolvedValue(null);
    const result = await getAddressByStreetAndNumber.execute(client_id, street, streetNumber);
    globalExpect(prisma.address.findFirst).toHaveBeenCalledWith({
      where: {
        street,
    ...
    globalExpect(result).toEqual(null);
  });
});
