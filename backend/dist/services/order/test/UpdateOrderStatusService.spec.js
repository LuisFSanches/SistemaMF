...
// src/services/order/test/UpdateOrderStatusService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
describe("UpdateOrderStatusService", () => {
  let updateOrderStatusService;
  let prisma;
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("should update order status", async () => {
    ...
    prisma.order.update.mockResolvedValue(mockedOrder);
    const result = await updateOrderStatusService.execute({
      id: mockedOrder.id,
      status: mockedOrder.status
    });
    globalExpect(prisma.order.update).toHaveBeenCalledWith({
      where: {
        id: mockedOrder.id
      },
      data: {
