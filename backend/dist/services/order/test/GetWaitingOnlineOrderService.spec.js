...
// src/services/order/test/GetWaitingOnlineOrderService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
describe("GetWaitingOnlineOrderService", () => {
  ...
  it("should get an waiting online order", async () => {
    ...
    globalExpect(prisma.order.findMany).toHaveBeenCalledWith({
      where: {
        status: "WAITING_FOR_CLIENT"
      },
      include: {
    ...
    globalExpect(result).toEqual({ orders: [mockedOrder] });
  });
});
