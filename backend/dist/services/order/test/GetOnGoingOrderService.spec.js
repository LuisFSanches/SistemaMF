...
// src/services/order/test/GetOnGoingOrderService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
describe("GetOnGoingOrderService", () => {
  let getOnGoingOrderService;
  beforeEach(() => {
  ...
  it("should return orders", async () => {
    ...
    prisma_default.order.findMany.mockResolvedValue(mockedOrders);
    const result = await getOnGoingOrderService.execute();
    globalExpect(prisma_default.order.findMany).toHaveBeenCalledWith({
      where: {
        status: {
          notIn: ["FINISHED", "CANCELED", "DONE"]
        }
    ...
    globalExpect(result).toEqual(mockedOrders);
  });
});
