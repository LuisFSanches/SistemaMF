...
// src/services/order/test/CreateOrderService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
...
describe("CreateOrderService", () => {
  ...
  it("should create an order successfully", async () => {
    ...
    prisma_default.order.create.mockResolvedValue(mockCreatedOrder);
    const result = await createOrderService.execute(mockOrderData, mockProducts);
    globalExpect(prisma_default.order.create).toHaveBeenCalledWith({
      data: {
        ...mockOrderData,
        delivery_date: /* @__PURE__ */ new Date("2023-08-01T15:00:00.000Z"),
        orderItems: {
    ...
    globalExpect(result).toEqual(mockCreatedOrder);
  });
});
