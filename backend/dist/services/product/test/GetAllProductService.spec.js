...
// src/services/product/test/GetAllProductService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
describe("GetAllProductService", () => {
  ...
  it("should get all products with default pagination", async () => {
    ...
    prisma_default.product.count.mockResolvedValue(2);
  ...
  it("should get products with custom pagination", async () => {
    ...
    prisma_default.product.findMany.mockResolvedValue(mockProducts);
    prisma_default.product.count.mockResolvedValue(25);
  ...
  it("should return error object if Prisma throws an exception", async () => {
    ...
    prisma_default.product.findMany.mockRejectedValue(new Error(errorMessage));
    try {
  ...
  it("should handle empty results", async () => {
    prisma_default.product.findMany.mockResolvedValue([]);
    prisma_default.product.count.mockResolvedValue(0);
    const result = await service.execute();
    globalExpect(result).toEqual({
      products: [],
      total: 0,
  ...
  it("should only return enabled products", async () => {
    ...
    globalExpect(result.products).toEqual(mockProducts);
  });
});
