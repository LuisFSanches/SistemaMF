...
// src/services/product/test/SearchProductsService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
describe("SearchProductsService", () => {
  ...
  it("should return empty array when query is empty", async () => {
    ...
    globalExpect(prisma_default.$queryRawUnsafe).not.toHaveBeenCalled();
  ...
  it("should return empty array when query is undefined", async () => {
    const result = await service.execute(void 0);
    globalExpect(prisma_default.$queryRawUnsafe).not.toHaveBeenCalled();
    globalExpect(result).toEqual([]);
  });
  ...
  it("should handle accent-insensitive search", async () => {
    ...
    globalExpect(prisma_default.$queryRawUnsafe).toHaveBeenCalledWith(
      globalExpect.stringContaining("unaccent(lower($1))"),
      "acai"
    );
    globalExpect(result).toEqual(mockProducts);
  });
  it("should return empty array when no products match", async () => {
    prisma_default.$queryRawUnsafe.mockResolvedValue([]);
  ...
  it("should limit results to 50 products", async () => {
    ...
    globalExpect(prisma_default.$queryRawUnsafe).toHaveBeenCalledWith(
      globalExpect.stringContaining("LIMIT 50"),
      "Product"
    );
    globalExpect(result).toHaveLength(50);
  });
  it("should only return enabled products", async () => {
    ...
    const result = await service.execute("Product");
    globalExpect(prisma_default.$queryRawUnsafe).toHaveBeenCalledWith(
      globalExpect.stringContaining("WHERE enabled = true"),
      "Product"
    );
    globalExpect(result).toEqual(mockProducts);
  });
