...
var GetClientService = class {
  ...
  generateSpendingHistory(orders) {
    ...
    for (let i = 11; i >= 0; i--) {
      const date = new Date(now3.getFullYear(), now3.getMonth() - i, 1);
...
// src/services/client/test/GetClientService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
describe("GetClientService", () => {
  ...
  it("should get a client by id successfully", async () => {
    ...
    globalExpect(result).toEqual({ user: mockClient });
  });
  it("should return user as null when client is not found", async () => {
    ...
    globalExpect(result).toEqual({ user: null });
  });
