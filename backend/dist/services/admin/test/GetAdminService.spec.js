...
// src/services/admin/GetAdminService.ts
var GetAdminService = class {
  async execute(id) {
    try {
...
// src/services/admin/test/GetAdminService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
describe("GetAdminService", () => {
  ...
  it("should get an admin", async () => {
    ...
    prisma_default.admin.findFirst.mockResolvedValue(mockAdmin);
    const result = await service.execute(adminId);
    globalExpect(prisma_default.admin.findFirst).toHaveBeenCalledWith({
      where: {
        id: adminId
      }
    });
