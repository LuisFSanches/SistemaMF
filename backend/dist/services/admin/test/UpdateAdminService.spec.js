...
// src/services/admin/test/UpdateAdminService.spec.ts
...
describe("UpdateAdminService", () => {
  ...
  beforeEach(() => {
    ...
    import_bcrypt2.hash.mockResolvedValue(mockHashedPassword);
  });
  it("should update an admin with password", async () => {
    ...
    prisma_default.admin.update.mockResolvedValue(mockAdmin);
    const adminInput = {
      id: adminId,
      username: "admin",
      name: "Admin",
    ...
    globalExpect(import_bcrypt2.hash).toHaveBeenCalledWith("plainPassword123", 10);
    globalExpect(prisma_default.admin.update).toHaveBeenCalledWith({
      where: {
        id: adminId
      },
      data: {
    ...
    globalExpect(result).toEqual(mockAdmin);
  });
  it("should update an admin without password", async () => {
    ...
    const result = await service.execute(adminInput);
    globalExpect(import_bcrypt2.hash).not.toHaveBeenCalled();
    globalExpect(prisma_default.admin.update).toHaveBeenCalledWith({
      where: {
        id: adminId
      },
  ...
  it("should handle database errors", async () => {
    ...
    prisma_default.admin.update.mockRejectedValue(new Error(errorMessage));
    const adminInput = {
      id: adminId,
      username: "admin",
      name: "Admin",
