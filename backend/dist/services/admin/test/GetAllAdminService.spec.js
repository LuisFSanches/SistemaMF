...
// src/services/admin/test/GetAllAdminService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
describe("GetAllAdminService", () => {
  ...
  it("should get all admins", async () => {
    ...
    prisma_default.admin.findMany.mockResolvedValue(mockAdmins);
    prisma_default.admin.count.mockResolvedValue(1);
    const result = await service.execute();
    globalExpect(prisma_default.admin.findMany).toHaveBeenCalledWith({
      select: {
        id: true,
        name: true,
        role: true,
    ...
    globalExpect(result).toEqual({ admins: mockAdmins });
  });
});
