...
// src/services/admin/test/DeleteAdminService.spec.ts
vi.mock("../../../prisma", () => ({
  default: mockDeep()
}));
describe("DeleteAdminService", () => {
  let deleteAdminService;
  let prisma;
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("should delete an admin successfully", async () => {
    ...
    globalExpect(prisma.admin.delete).toHaveBeenCalledWith({ where: { id: adminId } });
    globalExpect(result).toEqual({ Status: "Admin successfully deleted" });
  });
  it("should throw BadRequestException when admin is not found", async () => {
    ...
    await globalExpect(deleteAdminService.execute(adminId)).rejects.toThrow(BadRequestException);
    await globalExpect(deleteAdminService.execute(adminId)).rejects.toThrow(errorMessage);
    globalExpect(prisma.admin.delete).toHaveBeenCalledWith({ where: { id: adminId } });
  });
