...
// src/services/admin/test/CreateAdminService.spec.ts
...
describe("CreateAdminService", () => {
  let createAdminService;
  let prismaClientMock;
  const mockHashedPassword = "hashedPassword123";
  beforeEach(() => {
    vi.clearAllMocks();
    prismaClientMock = prisma_default;
  ...
  it("should create admin", async () => {
    ...
    globalExpect(import_bcrypt2.hash).toHaveBeenCalledWith("password123", 10);
    globalExpect(prismaClientMock.admin.findFirst).toHaveBeenCalledWith({
      where: {
        username: "admin"
      }
    });
    globalExpect(prismaClientMock.admin.create).toHaveBeenCalledWith({
      data: {
        username: "admin",
        name: "Admin",
        password: mockHashedPassword,
    ...
    globalExpect(result).toEqual(mockCreatedAdmin);
  });
});
