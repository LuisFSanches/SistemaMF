...
class UploadStoreProductImageService {
    async execute({ store_product_id, filename }: IUploadStoreProductImage) {
        ...
        try {
            if (useR2) {                
                ...
                if (storeProduct.image && storeProduct.image.includes(process.env.R2_PUBLIC_URL || '')) {
                    await r2Service.delete({ fileUrl: storeProduct.image });
                }
                ...
                if (storeProduct.image && !storeProduct.image.includes(process.env.R2_PUBLIC_URL || '')) {
                    const oldImagePath = storeProduct.image.replace(`${backendUrl}/uploads/products/`, '');
            ...
            console.error("[UploadStoreProductImageService] Failed:", error);
...
export { UploadStoreProductImageService };
