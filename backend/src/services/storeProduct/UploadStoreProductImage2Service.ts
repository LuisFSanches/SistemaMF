...
class UploadStoreProductImage2Service {
    async execute({ store_product_id, filename }: IUploadStoreProductImage) {
        ...
        try {
            if (useR2) {                
                ...
                if (storeProduct.image_2 && storeProduct.image_2.includes(process.env.R2_PUBLIC_URL || '')) {
                    await r2Service.delete({ fileUrl: storeProduct.image_2 });
                }
                ...
                if (storeProduct.image_2 && !storeProduct.image_2.includes(process.env.R2_PUBLIC_URL || '')) {
                    const oldImagePath = storeProduct.image_2.replace(`${backendUrl}/uploads/products/`, '');
            ...
            console.error("[UploadStoreProductImage2Service] Failed:", error);
...
export { UploadStoreProductImage2Service };
