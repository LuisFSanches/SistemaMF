...
interface IUploadStoreBannerMobile3 {
    store_id: string;
    filename: string;
}

class UploadStoreBannerMobile3Service {
    async execute({ store_id, filename }: IUploadStoreBannerMobile3) {
        ...
        try {
            if (useR2) {
                ...
                if (store.banner_mobile_3 && store.banner_mobile_3.includes(process.env.R2_PUBLIC_URL || '')) {
                    await r2Service.delete({ fileUrl: store.banner_mobile_3 });
                }
                ...
                if (store.banner_mobile_3 && !store.banner_mobile_3.includes(process.env.R2_PUBLIC_URL || '')) {
                    const oldBannerPath = store.banner_mobile_3.replace(`${backendUrl}/uploads/stores/`, '');
            ...
            const updatedStore = await prismaClient.store.update({
                where: { id: store_id },
                data: { banner_mobile_3: bannerUrl },
                select: {
                    id: true,
                    name: true,
                    slug: true,
            ...
            console.error("[UploadStoreBannerMobile3Service] Failed:", error);
...
export { UploadStoreBannerMobile3Service };
