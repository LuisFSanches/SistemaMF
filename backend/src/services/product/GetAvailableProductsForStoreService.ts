...
class GetAvailableProductsForStoreService {
	async execute(storeId: string, page: number = 1, pageSize: number = 8, query?: string) {
		try {
			if (!storeId) {
			...
			if (query && query.trim()) {
				...
				const queryParams = [...searchTerms, storeId, pageSize, skip];
				...
				const productsRaw = await prismaClient.$queryRawUnsafe<any[]>(
					`
						...
						AND NOT EXISTS (
							...
							WHERE sp.product_id = p.id AND sp.store_id = $${searchTerms.length + 1}
						)
			...
			console.error("[GetAvailableProductsForStoreService] Failed:", error);
			throw new BadRequestException(
				error.message,
...
export { GetAvailableProductsForStoreService }
